...
timestamp_in_ms());
	if (get_int(&philo->id_mutex, philo->id) % 2 == 0)
		usleep(get_long(&philo->var->time2eat_mutex, \
philo->var->time_to_eat) * 1000 * 0.5);
	else if (get_int(&philo->id_mutex, philo->id) == philo->var->num_of_philo)
	{
		usleep(get_long(&philo->var->time2eat_mutex, \
philo->var->time_to_eat) * 1000 * 0.6);
	}
	while (get_int(&philo->remaining_meals_mutex, philo->remaining_meals) && \
(!get_int(&philo->var->isdead_mutex, philo->var->is_dead)))
	{
		...
		set_timestamp(&philo->lastmeal_mutex, \
&philo->last_meal_timestamp, timestamp_in_ms());
		usleep(get_long(&philo->var->time2eat_mutex, \
philo->var->time_to_eat) * 1000);
		pthread_mutex_unlock(&philo->var->forks[philo->first_fork]);
		pthread_mutex_unlock(&philo->var->forks[philo->second_fork]);
		if (get_int(&philo->var->isdead_mutex, philo->var->is_dead))
			break ;
		write_status(SLEEPING, philo, DEBUG_MODE);
		usleep(get_long(&philo->var->time2sleep_mutex, \
philo->var->time_to_sleep) * 1000);
		if (get_int(&philo->var->isdead_mutex, philo->var->is_dead))
			break ;
